    ...
    std::size_t part)
{
  ...
  for (size_t i = 0; i != distance; i += 1)
  {
    ...
    size_t index = *(validators_indices_begin + i + offset);
    ...
    typename std::array<block_type, leaves_size * 20>::iterator changed_validators_beacon_proofs_begin)
{
  ...
  for (size_t i = 0; i != distance; i += 1)
  {
    ...
    block_type empty = {0, 0};
    block_type hashed = hash<sha_hash>(empty, empty);
...
constexpr size_t VALIDATORS_TREE_HEIGHT_BEACON = 20;
...
constexpr size_t MAX_VALIDATORS_CHANGED = 256;
...
[[circuit]] field_type recursive_circuit(
    ...
    [[private_input]] std::array<field_type, VALIDATORS_MAX_COUNT_BEACON * 10> validators_fields,
    [[private_input]] std::array<field_type, CHANGED_VALIDATORS_ARRAY_SIZE> changed_validators_fields,
    ...
    [[private_input]] uint64_t_be epoch)
{
  ...
  */
  ...
  std::array<field_type, 16384> layer0; // 16384 validators leaves
  std::array<field_type, 4> layer1;
  std::array<field_type, 2> balances_subtotals;
...
#pragma zk_multi_prover 1
  {
    ...
    compute_validator_leaves<1048576>(
        validators_begin,
        epoch,
        8192,
        layer0_begin,
