...
function getArrayOfFields(size: number): any {
  const array = [] as (typeof field)[];
  for (let i = 0; i < size; i++) {
    array.push(field);
  }
  return getArray(array);
...
function generateInput(validators: number): object {
  ...
  const validators_effective_balance = getArrayOfFields(validators);
  const validators_slashed = getArrayOfFields(validators);
  const validators_activation_eligibility_epoch = getArrayOfFields(validators);
  const validators_activation_epoch = getArrayOfFields(validators);
  const validators_exit_epoch = getArrayOfFields(validators);
  const validators_withdrawable_epoch = getArrayOfFields(validators);
...
// Generate the object
const input = generateInput(16384);
