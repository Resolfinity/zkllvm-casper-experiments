...
{
  // return block0[0] < block1[0]; doesn't work, unknown predicate error
    ...
    [[private_input]] std::array<size_t, VOTERS_COUNT> sorted_to_voters_mapping)
{
  ...
  for (size_t idx = 0; idx < VOTERS_COUNT - 1; ++idx)
  {
    ...
    if (is_validator_updated[validator_idx])
    {
      __builtin_assigner_exit_check(is_validator_active(updated_validators_activation_epoch[0], updated_validators_exit_epoch[0], epoch));
    }
    else
    {
      __builtin_assigner_exit_check(is_validator_active(validators_activation_epoch[0], validators_exit_epoch[0], epoch));
    }
